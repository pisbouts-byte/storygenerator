...
| Category | Labels (`category:foundation-overhead`, etc.) unless a custom field exists | Keeps functional vs. overhead work distinguishable post-import |
...
| Sizing Drivers | Description (dedicated "Sizing Drivers" heading) | Not usually a dedicated JIRA field — keep it visible in Description for grooming/audit purposes |
...
| Requirement Origin | Labels (`origin:sourced` / `origin:inferred`) unless a custom field exists | Keeps inferred vs. explicitly sourced requirements distinguishable post-import, not just in the workbook |
